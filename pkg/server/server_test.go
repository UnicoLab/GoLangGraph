...
func TestAgentManager(t *testing.T) {
	...
	// An omitted ID is generated by agent.NewAgent. The manager must index by
	// that generated value so clients can use the ID returned by the API.
	agentID := createdAgent.GetConfig().ID
	if agentID == "" {
		t.Fatal("created agent should receive an id")
	}
	if _, found := manager.GetAgent(agentID); !found {
		t.Fatalf("manager did not index generated id %q", agentID)
	}
